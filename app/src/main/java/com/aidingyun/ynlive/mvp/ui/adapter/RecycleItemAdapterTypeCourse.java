 ...
 */
public class RecycleItemAdapterTypeCourse extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private Context context;
    private List<SearchCourseModel.ListBean> results;

    //get & set
    public List<SearchCourseModel.ListBean> getResults() {
        return results;
    }

    public RecycleItemAdapterTypeCourse(Context context, List<SearchCourseModel.ListBean> results) {
    ...
    private void bind(ItemViewHolder holder, int position){
        ...
        holder.tv_grade.setText(results.get(position).getScore()+"分");
        holder.tv_collent.setText(results.get(position).getPeopel_count());
